#!/usr/bin/env bash
# install-all.sh — installs codemole + all dependencies
...
step "Checking Java (required for --lang java)"
...
install_java() {
    case "$PKG_MGR" in
        ...
        brew)    brew install openjdk@17
                 # Homebrew does not symlink by default
...
if has_cmd java; then
    ...
    if [ "${JAVA_VER:-0}" -ge 17 ] 2>/dev/null; then
        ok "java $(java -version 2>&1 | head -1) — OK"
        JAVA_OK=true
    else
...
step "Building and installing codemole"
...
ok "codemole installed to $CARGO_BIN/codemole"

if [ -f "$JAR_PATH" ]; then
    cp "$JAR_PATH" "$CARGO_BIN/java-parser.jar"
    ok "java-parser.jar copied alongside codemole in $CARGO_BIN"
fi
...
echo
echo "For Java projects, java-parser.jar is automatically found next to the executable."
echo "  codemole --lang java --endpoint /api/users --path ./my-project"
echo
